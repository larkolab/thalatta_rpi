...
static void log_csv(FILE * file, uint64_t gw_id, uint8_t * buf);
    ...
    while( ( quit_sig != 1 ) && ( exit_sig != 1 ) )
    {
        ...
        if( databuf_up[3] == PKT_PUSH_DATA )
        {
            if( log_fname != NULL )
            {
                if( is_first == true )
                {
                    fprintf(log_file, "gw,tmst,chan,rfch,freq,stat,modu,datr,bw,codr,rssic,lsnr,size,data\n");
                    is_first = false;
                }
                log_csv( log_file, gw_mac, &databuf_up[12] );
            }
        }
    }
...
static void log_csv(FILE * file, uint64_t gw_id, uint8_t * buf)
{
    ...
    if( rxpk_array != NULL)
    {
        ...
        for( i = 0; i < rxpk_nb; i++ )
        {
            fprintf(file, "%08X%08X", (uint32_t)( gw_id >> 32 ), (uint32_t)( gw_id & 0xFFFFFFFF ) );
